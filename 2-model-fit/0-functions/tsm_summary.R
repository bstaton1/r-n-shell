...
tsm_summary = function(post, params, model, diag_plots = T) {
  ...
  if (!is.null(post)) {
    ...
    mat = cbind(alpha_post, beta_post,sigma_R_post, 
                ...
                mgmt_post)
    if (model %in% c(3,4)) {
      mat = cbind(mat, D_sum = D_sum_post)
    }
    ...
    if (diag_plots) {
      pdf(paste("2-outputs/tsm_", model, ".pdf", sep = ""), h = 5, w = 8)
      ...
      if (model %in% 3:4) x = get.post(new_post, "D_sum", do.plot = T, new.window = F)
    ...
    ests = rbind(alpha_summ, beta_summ, sigma_R_summ,
                 ...
                 pi_summ, phi_summ, mgmt_summ)
    if (model %in% 3:4) {
      ests = rbind(ests, D_sum_summ)
    }
    id = data.frame( 
                    ...
                    method = paste("tsm", model, sep = ""), stringsAsFactors = F)
    if (model %in% 3:4) {
      id = rbind(id, data.frame(param = "D_sum", stock = NA, method = unique(id$method)))
    }
    ests = cbind(id, ests)
    ...
    output = data.frame(seed = seed, param = NA, stock = NA,
                        method = paste("tsm", model, sep = ""), mean = NA, sd = NA, 
                        x1 = NA, x2 = NA, x3 = NA)
