...
tsm_2_summary = function(post, params, seed, verbose = T, diag_plots = T) {
  
  # print message
  if(verbose) cat("  Summarizing TSM Model #2 Output", "\n", sep = "")
  ...
  if (!is.null(post)) {
    ...
    if (diag_plots) {
      pdf(fileName("Output/tsm_2_diag_plots", seed,".pdf"), h = 5, w = 8)
